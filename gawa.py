...
def effective_area_framed_tile(tile, data_fp, footprint, admin):
    ...
    pixels_in_disc = hp.query_disc(nside=Nside, nest=nest, 
                                   vec=hp.pixelfunc.ang2vec(np.radians(90.-tile['dec']), np.radians(tile['ra'])),
                                   radius = np.radians(radius_deg), inclusive=False)
    framed_eff_area_deg2 = np.sum(frac_map[np.isin(hpix_map, pixels_in_disc)]) * hp.pixelfunc.nside2pixarea(Nside, degrees=True)
...
def cmd_filter(magg, magr, cmd_mask, isochrone_masks):
    ...
    ixdata, iydata = xy2pix (color[cond_frame], mag[cond_frame], xmin, xmax, ymin, ymax, nx, ny)
...
def cmd_mask(dslice, isochrone_masks, out_paths):
    ...
    iy_HB = max( x2pix( np.array([gmag_HB(isochrone_masks, dslice)+1.]), ymin, ymax, ny)[0], 0)
    ...
    for iy in range(iy_HB, ny):
        ...
        ixarn = np.hstack(  (ixarn,   np.arange( np.amin(ixar[iyar==iy]), np.amax(ixar[iyar==iy])+1)))
        iyarn = np.hstack(  (iyarn,   iy*np.ones(np.amax(ixar[iyar==iy]) - np.amin(ixar[iyar==iy])+1).astype(int)))
...
def compute_cmd_masks(isochrone_masks, out_paths, gawa_cfg):
    ...
    #dslices = np.loadtxt(isochrone_masks['dslices_file'], usecols=(0), unpack=True)
    dslices = read_FitsCat( os.path.join(out_paths['workdir'], gawa_cfg['dslices']['dslices_filename']))['dist_pc']
    ...
    for i in range(0, len(dslices)):
        ...
        if not os.path.isfile(os.path.join(out_paths['workdir'], out_paths['masks'],\
                             'cmd_mask_D'+str(dslices[i]/1000)+'.npy')):
            mask = cmd_mask(dslices[i], isochrone_masks, out_paths)
            np.save(os.path.join(out_paths['workdir'], out_paths['masks'],\
...
def select_stars_in_slice(data_star, starcat, gawa_cfg, mask, isochrone_masks):
    ...
    magg, magr = data_star[starcat['keys']['key_mag_g']], data_star[starcat['keys']['key_mag_r']]
...
def pixelized_radec(ra_map, dec_map, weight_map, w, nxy):
    ...
    xycat, xedges, yedges = np.histogram2d(xmap, ymap, bins=nxy, range=((0,nxy),(0,nxy)), normed=None, weights=weight_map)
    return xycat
...
def pixelized_colmag(color, mag, weight, isochrone_masks, out):
    ...
    colmag_pix, cedges, medges = np.histogram2d(xmap, ymap, bins=(nx, ny), range=((0,nx),(0,ny)), normed=None, weights=weight)
...
def plot_pixelized_colmag(color_aper, gmag_aper, weight_aper, color_slaper, gmag_slaper, weight_slaper, color, mag, weight, isochrone_masks, out):
    xmin, xmax = isochrone_masks["mask_color_min"], isochrone_masks["mask_color_max"]
    ymin, ymax = isochrone_masks["mask_mag_min"],   isochrone_masks["mask_mag_max"]
    step = 0.05
    nx = int((xmax-xmin)/step)
    ...
    plt.scatter(color_slaper, gmag_slaper, s=15, c='red', alpha=1, label= "stars in aper + mask") 
...
def randoms_in_spherical_cap(tile, bkg_arcmin2):
    ...
    for i in range(0, 20):
        ...
        if pdens >= bkg_arcmin2:
            Nside_samp = nside#*2
            break
    pixels_in_disc = hp.query_disc(nside=Nside_samp, nest=False, 
                                   vec=hp.pixelfunc.ang2vec(np.radians(90.-tile['dec']), np.radians(tile['ra'])),
                                   radius = np.radians(tile['radius_tile_deg']), inclusive=False)
...
def compute_filled_catimage(ra_map, dec_map, weight_map, gawa_cfg, tile, data_fp, footprint, bkg_arcmin2):

    # find edge pixels of the footprint 
    nlist = hp.pixelfunc.get_all_neighbours(footprint['Nside'], data_fp[footprint['key_pixel']], None, footprint['nest'])
    ...
    shpix_filled = np.unique(radec2hpix(ra_map, dec_map, footprint['Nside']*2, footprint['nest']))
    ...
    nlist_esep = hp.pixelfunc.get_all_neighbours(2*footprint['Nside'], empty_sub_edge_hpix, None, footprint['nest'])
...
def run_mr_filter(filled_catimage, wmap, gawa_cfg):
    ...
    scale_min_pix = gawa_cfg['wavelet_specs'][gawa_cfg['detection_mode']]['scale_min_arcmin'] * float(gawa_cfg['map_resolution'])
    scale_max_pix = gawa_cfg['wavelet_specs'][gawa_cfg['detection_mode']]['scale_max_arcmin'] * float(gawa_cfg['map_resolution'])
    smin = int(round(math.log10(scale_min_pix)/math.log10(2.)))
    smax = int(round(math.log10(scale_max_pix)/math.log10(2.)))
    if smin == 0:
        os.system(os.path.join(path_mr, 'mr_filter')+' -m 10 -i 3 -s 3.,3. -n '+str(smax+1)+' -f 3 -K -C 2 -p -e0 -A '+filled_catimage+' '+wmap)
    if smin == 1:
        os.system(os.path.join(path_mr, 'mr_filter')+' -m 10 -i 3 -s 10.,3.,3. -n '+str(smax+1)+' -f 3 -K -C 2 -p -e0 -A '+filled_catimage+' '+wmap)
    if smin == 2:
        os.system(os.path.join(path_mr, 'mr_filter')+' -m 10 -i 3 -s 10.,10.,3.,3. -n '+str(smax+1)+' -f 3 -K -C 2 -p -e0 -A '+filled_catimage+' '+wmap)
    if smin == 3:
        os.system(os.path.join(path_mr, 'mr_filter')+' -m 10 -i 3 -s 10.,10.,10.,3.,3. -n '+str(smax+1)+' -f 3 -K -C 2 -p -e0 -A '+filled_catimage+' '+wmap)
    return
...
def wmap2peaks(wmap, wazp_specs, tile_specs):
    ...
    local_maxi = peak_local_max(wmap_data, min_distance=8)  # this has to be reviewed and configured !
    npeaks0 = len(local_maxi) # nr of peaks in the filled_map 
    iobj0, jobj0 = local_maxi[:,0]+1., local_maxi[:,1]+1. # pixel coordinates of the peaks
...
def wave_radius(wmap_data, ip, jp, gawa_cfg):
    ...
    dwmap = ndi.distance_transform_edt((wmap_data>gawa_cfg['wmap_thresh'])*wmap_data)
...
def filter_peaks(tile, map_resolution, ra0, dec0, ip0, jp0):
    if tile['hpix']>0: # not target mode
        ...
        ra =   ra0[np.degrees(dist_ang(ra0, tile['ra'], dec0, tile['dec']))<=tile['radius']]
        dec = dec0[np.degrees(dist_ang(ra0, tile['ra'], dec0, tile['dec']))<=tile['radius']]
        ip =   ip0[np.degrees(dist_ang(ra0, tile['ra'], dec0, tile['dec']))<=tile['radius']]
        jp =   jp0[np.degrees(dist_ang(ra0, tile['ra'], dec0, tile['dec']))<=tile['radius']]
    return    ra, dec, ip, jp 
...
def coverfrac_disc(ra, dec, data_footprint, footprint, radius_deg):
    ...
    for i in range(0, len(ra)):
        hpx_in_ann, frac_in_ann, area_deg2, coverfrac[i] = hpx_in_annulus (ra[i], dec[i], 0., radius_deg, data_footprint, footprint, False)
        #print (i, ra[i], dec[i], coverfrac[i], frac_in_ann)
    return coverfrac


def init_peaks_table(ra_peaks, dec_peaks, iobj, jobj, coverfrac, wradius, dslice, isochrone_masks, gawa_cfg):
    """
    ...
    data_peaks = np.zeros( (len(ra_peaks)), 
          dtype={
              'names':('peak_id', 'ra', 'dec', 'iobj', 'jobj', 'dist_init_kpc', 'dist_err_kpc', \
                       'dist_min_kpc', 'dist_max_kpc', 'coverfrac', gawa_cfg['clkeys']['key_radius']),
              'formats':('i8', 'f8', 'f8', 'f4', 'f4', 'f4', 'f4', 'f4', 'f4', 'f4', 'f4')}) 
...
def select_stars_in_disc(racen, deccen, data_stars, starcat, aper):
    ras, decs = data_stars[starcat['keys']['key_ra']], data_stars[starcat['keys']['key_dec']]
...
def compute_dslices(isochrone_masks, dslices_specs, workdir):
    ...
    data = np.zeros( (len(dslices)), dtype={'names':('id', 'dist_pc'), 'formats':('i8', 'f8')})
...
def excess_cmd_flux(rap, decp, data_stars, starcat, isochrone_masks, aper, colmag_pix_tile, dslice, gawa_cfg):
    ...
    for i in range(0, len(rap)):
        data_star_aper = select_stars_in_disc(rap[i], decp[i], data_stars, starcat, aper/60.)
        gmag = data_star_aper[starcat['keys']['key_mag_g']]
        rmag = data_star_aper[starcat['keys']['key_mag_r']]
        color = gmag-rmag
        colmag_pix = pixelized_colmag(color, gmag, np.ones(len(rmag)), isochrone_masks, None)
        excess = colmag_pix - aper_area * colmag_pix_tile
        wexcess = excess*weight_map
        Naper[i] = np.sum(excess[excess>0.])
        Naperw[i] = np.sum(wexcess[wexcess>0.])
    return Naper, Naperw


def add_peaks_attributes(data_peaks, data_stars, starcat, bkg_arcmin2, dslice, isochrone_masks, tile, gawa_cfg):
    ...
    Nraw_snr = compute_flux_aper_vec(data_peaks['ra'], data_peaks['dec'], gawa_cfg['radius_aper_arcmin']/60.,\
                                     ras, decs, np.ones(len(ras)))

    Naper_weighted = compute_flux_aper_vec(data_peaks['ra'], data_peaks['dec'], gawa_cfg['radius_aper_arcmin']/60.,\
                                           ras, decs, weights)
...
def append_peaks_infos_to_cylinders(data_cylinders_init, peaks_list, dslices,
    ...
    dtype={
              'names':('ra', 'dec', 'dist_init_kpc', 'snr', 'ip_cyl', 'ra_cyl', 'dec_cyl', 
                       'rank_cyl', 'snr_cyl'),
              'formats':('f8','f8','f8', 'f8', str(nsl)+'i8', str(nsl)+'f8', str(nsl)+'f8', 
...
def make_cylinders(peaks_list, dslices, gawa_cfg):
        ...
        if flag_min == 1:
            ...
            if isl == islmin: # all peaks are new cylinders 
                ...
                if len(peaks_list[isl-1])== 0: # no detection in intermediate slice 
                    ...
                    rank_cyl_new, snr_cyl_new = np.ones((np0,nsl))*(-1), np.ones((np0,nsl))*(-1)
                    ra_cyl_new, dec_cyl_new = np.ones((np0,nsl))*(-1), np.ones((np0,nsl))*(-1)
                    rank_cyl_new[:,isl], snr_cyl_new[:,isl] = dat[key_rank], dat[key_snr]
                    ra_cyl_new[:,isl], dec_cyl_new[:,isl] = dat['ra'], dat['dec']

                    ip_cyl = np.concatenate((ip_cyl,ip_cyl_new))
                    rank_cyl, snr_cyl = np.concatenate((rank_cyl,rank_cyl_new)), np.concatenate((snr_cyl,snr_cyl_new))
                    ra_cyl, dec_cyl = np.concatenate((ra_cyl,ra_cyl_new)), np.concatenate((dec_cyl,dec_cyl_new))
                else:
                    ...
                    i0,i1,sep2d,dist3d = astropy.coordinates.search_around_sky(c0, c1, rad_deg*u.degree, storekdtree='kdtree_sky')
    ...
    if npeaks_all > 0:
        ...
        data_cylinders = append_peaks_infos_to_cylinders(data_init, peaks_list, dslices, 
                                                         ip_cyl, ra_cyl, dec_cyl, rank_cyl, snr_cyl)    
        ncyl = len(data_cylinders)
        print('')
        print('..........Number of cylinders : '+str(ncyl))
        print('..........Ratio npeaks / ncyl : '+str(np.round(float(npeaks_all)/float(ncyl), 2)) )
    else:
        data_cylinders = None 
...
def gawa_tile_slice(tile, dslice, isochrone_masks, data_star, starcat, data_fp, footprint, gawa_cfg, out_paths, verbose):

    # select objects for computing density maps 
    cmd_mask = np.load(os.path.join(out_paths['workdir'], out_paths['masks'],\
                             'cmd_mask_D'+str(dslice/1000)+'.npy'))    
    data_star_slice = select_stars_in_slice(data_star, starcat, gawa_cfg, cmd_mask, isochrone_masks)
    ras, decs = data_star_slice[starcat['keys']['key_ra']], data_star_slice[starcat['keys']['key_dec']]
    gmag = data_star_slice[starcat['keys']['key_mag_g']]
    rmag = data_star_slice[starcat['keys']['key_mag_r']]
    cmd_histogram = os.path.join(out_paths['tile_dir'], out_paths['gawa']['plots'], 'cmd_hist_D'+str(dslice/1000)+'.png')
    pixelized_colmag(gmag-rmag, gmag, np.ones(len(gmag)), isochrone_masks, cmd_histogram)

    xycat = compute_catimage(ras, decs, np.ones(len(ras)), gawa_cfg, tile) 
    bkg_arcmin2 = float(len(ras))/(tile['framed_eff_area_deg2']*3600.)
    xycat_fi = compute_filled_catimage(ras, decs, np.ones(len(ras)), gawa_cfg, tile, 
                                       data_fp, footprint, bkg_arcmin2)

    # build density map /  extract peaks /compute attributes and filter 
    xycat_fi_fitsname = os.path.join(out_paths['tile_dir'], out_paths['gawa']['files'], \
                                     'xycat_fi_D'+str(dslice/1000)+'.fits')
    wmap_fitsname = os.path.join(out_paths['tile_dir'], out_paths['gawa']['files'], \
    ...
    rap, decp, ip, jp = filter_peaks(tile, gawa_cfg['map_resolution'], rap0, decp0, ip0, jp0) # to keep inner tile peaks
    wradius_arcmin = wave_radius(wmap_data, ip.astype(int), jp.astype(int), gawa_cfg)

    coverfrac = coverfrac_disc(rap, decp, data_fp, footprint, gawa_cfg['radius_aper_arcmin']/60.)
    pcond = ((coverfrac>0.3))
    data_peaks = init_peaks_table(rap[pcond], decp[pcond], ip[pcond], jp[pcond], coverfrac[pcond], \
                                  wradius_arcmin[pcond], dslice, isochrone_masks, gawa_cfg)
    
    # compute fluxes in given aper 
    add_peaks_attributes(data_peaks, data_star_slice, starcat, bkg_arcmin2, dslice, isochrone_masks, tile, gawa_cfg)

    # peaks to be kept
    condf = ( (data_peaks['snr']>=gawa_cfg['snr_min']) & (data_peaks['Naper']>=gawa_cfg['N_min']))


    # plot pixelized CMD diagrams bkg + stars in aper 
    if verbose >=2:
        for i in range(0, len(data_peaks[condf])):
            data_star_slice_aper = select_stars_in_disc(data_peaks[condf][i]['ra'], data_peaks[condf][i]['dec'], \
                                                        data_star_slice, starcat, gawa_cfg['radius_aper_arcmin']/60.)
            data_star_aper = select_stars_in_disc(data_peaks[condf][i]['ra'], data_peaks[condf][i]['dec'], \
                                                  data_star, starcat, gawa_cfg['radius_aper_arcmin']/60.)
            gmag_slaper, rmag_slaper = data_star_slice_aper[starcat['keys']['key_mag_g']], data_star_slice_aper[starcat['keys']['key_mag_r']]
            color_slaper = gmag_slaper-rmag_slaper
            gmag_aper, rmag_aper = data_star_aper[starcat['keys']['key_mag_g']], data_star_aper[starcat['keys']['key_mag_r']]
            color_aper = gmag_aper-rmag_aper
            cmd_histogram = os.path.join(out_paths['tile_dir'], out_paths['gawa']['plots'], 'cmd_hist_D'+str(dslice/1000)+'_radec'+\
                                         str(np.round(data_peaks[condf][i]['ra'],3))+'_'+str(np.round(data_peaks[condf][i]['dec'],3))+'.png')
            plot_pixelized_colmag(color_aper, gmag_aper, np.ones(len(gmag_aper)), \
    ...
    if verbose >=1:
        ...
        print ('..............peaks filtering coverfrac  in / out ', len(rap) , len(rap[pcond]))
        print ('..............peaks filtering SNR/N      in / out ', len(data_peaks), len(data_peaks[condf]))
        print ('         distance / density/arcmin2 = ', np.round(dslice/1000., 2), ' / ', np.round(bkg_arcmin2, 3))

    if verbose >=2:
        xycat_fitsname = os.path.join(out_paths['tile_dir'], out_paths['gawa']['files'], 'xycat_D'+str(dslice/1000)+'.fits')
        map2fits(xycat, gawa_cfg, tile, xycat_fitsname)
        weights = gmag_weight(data_star_slice[starcat['keys']['key_mag_g']], dslice, isochrone_masks, gawa_cfg)   
        t = Table (data_star_slice)
        t['weights'] = weights
        t.write(os.path.join(out_paths['tile_dir'], out_paths['gawa']['files'], 'starcat_D'+str(dslice/1000)+'.fits'),overwrite=True)    
    return data_peaks[condf]


def gawa_tile(tile_specs, isochrone_masks, data_star, starcat, data_fp, footprint, gawa_cfg, admin, out_paths, verbose):

    dslices = read_FitsCat( os.path.join(out_paths['workdir'], gawa_cfg['dslices']['dslices_filename']) )['dist_pc']

    print ('..........Start gawa tile catalog construction')

    if not os.path.isfile(os.path.join(out_paths['tile_dir'], out_paths['gawa']['results'], 'clusters0.fits')):
        peaks_list = []
        for isl in range (0, len(dslices)):
            if not os.path.isfile(os.path.join(out_paths['tile_dir'], out_paths['gawa']['files'], 'peaks_'+str(isl)+'.npy')):
                print ('.............. Detection in slice ', isl)
                data_peaks = gawa_tile_slice(tile_specs, dslices[isl], isochrone_masks, data_star, starcat, \
                                                 data_fp, footprint, gawa_cfg, out_paths, verbose)
                np.save(os.path.join(out_paths['tile_dir'], out_paths['gawa']['files'], 'peaks_'+str(isl)+'.npy'), data_peaks)
            else:
                print ('.............. Use existing detections in slice ', isl)
                data_peaks = np.load(os.path.join(out_paths['tile_dir'], out_paths['gawa']['files'], 'peaks_'+str(isl)+'.npy'))
            peaks_list.append(data_peaks)

        print ('..........Start cylinders')
        data_cylinders = make_cylinders(peaks_list, dslices, gawa_cfg )
        if verbose >=1 and data_cylinders!= None:
            t = Table (data_cylinders)#, names=names)
            t.write(os.path.join(out_paths['tile_dir'], out_paths['gawa']['results'], "cylinders.fits"), overwrite=True)

        if data_cylinders!= None:    
            print ('..........Start cylinders_2_clusters')
            data_clusters0 = cylinders2clusters(data_cylinders, peaks_list, tile_specs, dslices, out_paths, gawa_cfg['clkeys'])
        ...
        if Nclusters > 0:
            ...
            t.write(os.path.join(out_paths['tile_dir'], out_paths['gawa']['results'], "clusters0.fits"),overwrite=True)

    else:
        data_clusters0 = read_FitsCat(os.path.join(out_paths['tile_dir'], out_paths['gawa']['results'], 'clusters0.fits'))
        Nclusters = len(data_clusters0)

    if Nclusters > 0:
        data_clusters = cl_duplicates_filtering(data_clusters0, gawa_cfg, 'tile')
        t = Table (data_clusters0)#, names=names)
        t.write(os.path.join(out_paths['tile_dir'], out_paths['gawa']['results'], "clusters.fits"),overwrite=True)
    ...
    tile_info.write(os.path.join(out_paths['tile_dir'], out_paths['gawa']['results'], "tile_info.fits"), overwrite=True)

    return


#def cylinders2clusters (data_cyl):
#    best_slice = np.argmax(data_cyl['snr_cyl'], axis=1)
#    return data_cyl[np.arange(len(data_cyl)), best_slice]
...
def cylinders2clusters(data_cylinders, peaks_list, tile, dslices, out_paths, clkeys):
    ...
    for isl in range(0, len(dslices)):
        if len(peaks_list[isl]) > 0:
            ...
            data_peaks_ext['icyl'] = data_cylinders['id_cyl'][data_cylinders['cyl_isl_mode']==isl]
...
def cl_duplicates_filtering(data_clusters_in, gawa_cfg, mode):
    ...
    for i in range(0, len(data_cl)):
        if flagdp[i] == 0:
            ...
            if mode == 'tile':
                cond = ( (abs(distpc[i]-distpc)<= min_delta_distkpc) & (clid[i]!=clid) )
            if mode == 'survey':
                cond = ( (abs(distpc[i]-distpc)<= min_delta_distkpc) & (tile_id[i]!=tile_id) )
            in_cone = cond_in_disc(ra[cond], dec[cond], clhpx[cond], 
                                       Nside_tmp, nest_tmp,
