...
def read_mosaicFitsCat_in_disc (galcat, tile, radius_deg):
    ...
    fits_pixels_in_disc = hp.query_disc(nside=Nside_fits, nest=nest_fits, 
                                        vec=hp.pixelfunc.ang2vec(np.radians(90.-deccen), np.radians(racen)),
    ...
    if len(relevant_fits_pixels) > 0:
        ...
        for i in range (0, len(relevant_fits_pixels)):
            ...
            dcen = np.degrees( dist_ang(racen, deccen, dat_disc[galcat['keys']['key_ra']], dat_disc[galcat['keys']['key_dec']]))
            if i == 0:
...
def read_mosaicFootprint_in_disc (footprint, tile, radius_deg):
    ...
    raw_list = np.array(os.listdir(gdir))
    hpix_fits = np.array([os.path.splitext(x)[0].replace('_footprint','') for x in raw_list]).astype(int)
    extension = os.path.splitext(raw_list[0])[1]
    # find list of fits intersection cluster field
    ...
    if len(relevant_fits_pixels) > 0:
        # merge intersecting fits 
        for i in range (0, len(relevant_fits_pixels)):
            dat_disc = read_FitsCat(os.path.join(gdir, str(relevant_fits_pixels[i])+'_footprint'+extension))
            ra, dec = hpix2radec(dat_disc[footprint['key_pixel']], footprint['Nside'], footprint['nest'])
...
def read_mosaicFootprint_in_hpix (footprint, hpix_tile, Nside_tile, nest_tile):
    ...
    if len(relevant_fits_pixels) > 0:
        # merge intersecting fits 
        for i in range (0, len(relevant_fits_pixels)):
            dat = read_FitsCat(os.path.join(gdir, str(relevant_fits_pixels[i])+'_footprint'+extension))
            if i == 0:
...
def create_mosaic_footprint(footprint, fpath):
    ...
    hpix = radec2hpix(ra0, dec0, footprint['mosaic']['Nside'], footprint['mosaic']['nest'])
    for hp in np.unique(hpix):
        all_cols = fits.ColDefs([
            fits.Column(name=footprint['key_pixel'],  format='K',array= hpix0[np.isin(hpix, hp)]),
            fits.Column(name='ra',       format='E',array= ra0[np.isin(hpix, hp)]),
            fits.Column(name='dec',      format='E',array= dec0[np.isin(hpix, hp)]),
            fits.Column(name=footprint['key_frac'],   format='K',array= frac0[np.isin(hpix, hp)])])
...
def area_ann_deg2(theta_1, theta_2):
    return  2. * np.pi * (np.cos(np.radians(theta_1)) - np.cos(np.radians(theta_2))) * (180./np.pi)**2
...
def all_hpx_in_annulus (ra, dec, radius_in_deg, radius_out_deg, hpx_meta, inclusive):
    ...
    pixels_in_disc = hp.query_disc(nside=Nside, nest=nest, 
                                   vec=hp.pixelfunc.ang2vec(np.radians(90.-dec), np.radians(ra)),
                                   radius = np.radians(radius_out_deg), inclusive=inclusive)
    if radius_in_deg>0.:
        pixels_in_disc_in = hp.query_disc(nside=Nside, nest=nest, 
                                          vec=hp.pixelfunc.ang2vec(np.radians(90.-dec), np.radians(ra)),
                                          radius = np.radians(radius_in_deg), inclusive=inclusive)
...
def split_survey_from_hpx (hpx_footprint, hpx_meta, eff_tile_width, tile_overlap, output):
    ...
    size_pix_deg = hp.pixelfunc.nside2resol(Nside, arcmin=True)/60.
    ra_1, ra_2 = ra - size_pix_deg/np.cos(np.radians(dec)), ra + size_pix_deg/np.cos(np.radians(dec))
    ...
    if decmax-decmin < eff_tile_width:
        ...
        if decmax - dec_i[ny0-1] >= eff_tile_width/2. :
    ...
    deccen_i = (decmin_i+decmax_i)/2.
    cdec_frame0 = np.maximum(np.cos(decmin_i*np.pi/180.), np.cos(decmax_i*np.pi/180.))
    cdec0 = np.cos(deccen_i*np.pi/180.)

    # init 
    nx = np.zeros(ny).astype(int)        
    racen, deccen, cdec, cdec_frame = np.zeros(1), np.zeros(1), np.zeros(1), np.zeros(1)
    ramin_tile, ramax_tile, decmin_tile, decmax_tile = np.zeros(1), np.zeros(1), np.zeros(1), np.zeros(1)
    ...
    for i in range (0, ny):
        ...
        ramin_sl[i], ramax_sl[i] = survey_ra_minmax(ra[(dec<decmax_i[i]) & (dec>decmin_i[i])])
        ...
        if ramax_sl[i]-ramin_sl[i] < eff_tile_width:
            ...
            if ramax_sl[i] - ra_j[nx0-1] >= eff_tile_width/2. :
    ...
    size_frame_ra = (size_ra + 2.*tile_overlap)*cdec_frame/cdec
    size_frame_dec = size_dec + 2.*tile_overlap
    size_frame_i = np.maximum(size_frame_ra, size_frame_dec)
    size_frame = np.amax(size_frame_i)

    ramin_frame, ramax_frame = racen - size_frame/ (2.* cdec), racen + size_frame/ (2.* cdec)
    decmin_frame, decmax_frame = deccen - size_frame/ 2., deccen + size_frame/ 2.
    ...
    all_cols = fits.ColDefs([
        ...
        fits.Column(name='decmin_tile', format='E',array=np.around(decmin_tile,4)),
        fits.Column(name='decmax_tile', format='E',array=np.around(decmax_tile,4)),

        fits.Column(name='ramin_frame', format='E',array=np.around(ramin_frame,4)),
        fits.Column(name='ramax_frame', format='E',array=np.around(ramax_frame,4)),
        fits.Column(name='decmin_frame', format='E',array=np.around(decmin_frame,4)),
        fits.Column(name='decmax_frame', format='E',array=np.around(decmax_frame,4)),
        ...
        fits.Column(name='frame_size', format='E',array=np.around(size_frame_arr,4))])
...
def filter_hpx_tile(dat, keys, survey_tiling, hpix_tile):
    Nside, nest = survey_tiling['Nside'], survey_tiling['nest']
    hpix_gal = radec2hpix (dat[keys['key_ra']], dat[keys['key_dec']], Nside, nest) 
    return dat[hpix_gal==hpix_tile]

def filter_tile(dat, galcat_keys_dict, tiles_specs):
    
    # read tiles
    ramin_frame, ramax_frame   = tiles_specs["ramin_frame"],  tiles_specs["ramax_frame"]
    decmin_frame, decmax_frame = tiles_specs["decmin_frame"], tiles_specs["decmax_frame"]
    ...
    if ramin_frame < 0.:
        ra2 = ra-360.
        dat_tile1 = dat [(ra<=ramax_frame) & (ra>=ramin_frame) & (dec<=decmax_frame) & (dec>=decmin_frame)]
        dat_tile2 = dat [(ra2<=ramax_frame) & (ra2>=ramin_frame) & (dec<=decmax_frame) & (dec>=decmin_frame)]
        dat_tile = np.append(dat_tile1, dat_tile2)
    else:
        dat_tile = dat [(ra<=ramax_frame) & (ra>=ramin_frame) & (dec<=decmax_frame) & (dec>=decmin_frame)]
...
def hpx_split_survey (footprint_file, footprint, admin, output):
    ...
    '''
    hpix_tile, frac_tile = hpx_degrade(hpix_map, Nside_fp, nest_fp, Nside_tile, nest_tile)
    ...
    for i in range(0, len(racen)):
        pixels_in_disc = hp.query_disc(nside=Nside_fp, nest=nest_fp, 
                                       vec=hp.pixelfunc.ang2vec(np.radians(90.-deccen[i]), np.radians(racen[i])),
                                   radius = np.radians(radius_deg), inclusive=False)
        framed_eff_area_deg2[i] = np.sum(frac_map[np.isin(hpix_map, pixels_in_disc)]) * hp.pixelfunc.nside2pixarea(Nside_fp, degrees=True)

    data_tiles = np.zeros( (len(hpix_tile)), 
                           dtype={'names':('id', 'hpix', 'ra', 'dec', 'area_deg2', 'eff_area_deg2', \
                                           'framed_eff_area_deg2','radius_tile_deg', 'Nside', 'nest'), 
    ...
    t = Table (data_tiles)
    t.write(output, overwrite=True)
    print ('.....tile area (deg2) = ', np.round(hp.pixelfunc.nside2pixarea(Nside_tile, degrees=True), 2))
    print ('.....effective survey area (deg2) = ', np.around(np.sum(frac_tile)*area_tile,4))
...
def tile_radius(tiling):
    ...
    tile_radius = (2.*hp.pixelfunc.nside2pixarea(Nside_tile, degrees=True))**0.5 / 2.
...
def cond_in_disc(rag, decg, hpxg, Nside, nest, racen, deccen, rad_deg):
    ...
    pixels_in_disc_strict = hp.query_disc(nside=Nside, nest=nest, 
                                   vec=hp.pixelfunc.ang2vec(np.radians(90.-deccen), np.radians(racen)),
                                          radius = np.radians(rad_deg), inclusive=False)
    pixels_in_disc = hp.query_disc(nside=Nside, nest=nest, 
                                   vec=hp.pixelfunc.ang2vec(np.radians(90.-deccen), np.radians(racen)),
                                   radius = np.radians(rad_deg), inclusive=True)
...
def concatenate_clusters(tiles_dir, clusters_outfile): 
    ...
    for tile_dir in tiles_dir: 
        if os.path.isfile(os.path.join(tile_dir, "tile_info.fits")):
            if read_FitsCat(os.path.join(tile_dir, "tile_info.fits"))[0]['Nclusters'] > 0:
