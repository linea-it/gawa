...
def pixelized_radec(ra_map, dec_map, weight_map, w, nxy):
    ...
    xycat, xedges, yedges = np.histogram2d(
        ...
        normed=None, weights=weight_map
    )
    return xycat
...
def compute_filled_catimage(ra_map, dec_map, weight_map, gawa_cfg, tile, data_fp, 
    ...
    shpix_filled = np.unique(radec2hpix(
        ra_map, dec_map, footprint['Nside']*2, footprint['nest']
    ))
    hpix_filled = np.unique(radec2hpix(
...
def filter_peaks(tile, map_resolution, ra0, dec0, ip0, jp0):
    ...
    if tile['hpix']>0: # not target mode
        ...
        ra = ra0[np.degrees(dist_ang(\
                ra0, tile['ra'], dec0, tile['dec'])) <= tile['radius']]
        dec = dec0[np.degrees(dist_ang(\
                ra0, tile['ra'], dec0, tile['dec'])) <= tile['radius']]
        ip = ip0[np.degrees(dist_ang(\
                ra0, tile['ra'], dec0, tile['dec']))<=tile['radius']]
        jp = jp0[np.degrees(dist_ang(\
                ra0, tile['ra'], dec0, tile['dec']))<=tile['radius']]
    return    ra, dec, ip, jp 
...
def compute_dslices(isochrone_masks, dslices_specs, workdir):
    ...
    t.write(
        os.path.join(workdir, dslices_specs['dslices_filename'] ), overwrite=True
    )
    return
...
def gawa_tile_slice(tile, dslice, isochrone_masks, data_star, starcat, 
    ...
    rap, decp, ip, jp = filter_peaks(   # keep inner tile peaks
        tile, gawa_cfg['map_resolution'], 
        rap0, decp0, ip0, jp0
    )
    wradius_arcmin = wave_radius(wmap_data, ip.astype(int), jp.astype(int), gawa_cfg)
    coverfrac = coverfrac_disc(
        rap, decp, data_fp, footprint, gawa_cfg['radius_aper_arcmin']/60.
    )
    coverfrac_bkg = coverfrac_disc(
        rap, decp, data_fp, footprint, gawa_cfg['radius_bkg_max_arcmin']/60.
    )
