...
def get_logger(name=None, stdout="", level="info"):
    """
    Returns a logger object

    Args:
        name (string, optional): logger name. Defaults to None.
        stdout (string, optional): print to stdout. Defaults to ''.
...
def create_mosaic_footprint(footprint, fpath):
    ...
    for hpi in np.unique(hpix):
        all_cols = fits.ColDefs(
            [
                fits.Column(
                    name=footprint["key_pixel"],
                    format="K",
                    array=hpix0[np.isin(hpix, hpi)],
                ),
                fits.Column(name="ra", format="E", array=ra0[np.isin(hpix, hpi)]),
                fits.Column(name="dec", format="E", array=dec0[np.isin(hpix, hpi)]),
                fits.Column(
                    name=footprint["key_frac"],
                    format="K",
                    array=frac0[np.isin(hpix, hpi)],
                ),
            ]
        )
        hdu = fits.BinTableHDU.from_columns(all_cols)
        hdu.writeto(os.path.join(fpath, str(hpi) + "_footprint.fits"), overwrite=True)
    return
...
def all_hpx_in_annulus(ra, dec, radius_in_deg, radius_out_deg, hpx_meta, inclusive):
    ...
    if radius_in_deg > 0.0:
        ...
        id_annulus = np.isin(
            pixels_in_disc, pixels_in_disc_in, assume_unique=True, invert=True
        )
...
def cond_in_disc(rag, decg, hpxg, Nside, nest, racen, deccen, rad_deg):
    ...
    pixels_edge = pixels_in_disc[
        np.isin(pixels_in_disc, pixels_in_disc_strict, assume_unique=True)
    ]
